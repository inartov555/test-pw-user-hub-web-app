...
def test_login_success_regular(base_url, page):
    """User can log in with valid credentials and lands on a post-login page."""
...
def test_login_success_admin(base_url, page):
    """Admin can log in successfully; URL should be gated area."""
    lp = LoginPage(page, base_url)
    lp.open()
...
def test_login_remember_me_sets_persistent_storage(base_url, page):
    """Remember me should persist auth (localStorage/cookie)."""
    lp = LoginPage(page, base_url)
    lp.open()
    lp.login(USERS["regular2"]["username"], USERS["regular2"]["password"], remember=True)

    state = page.context.storage_state()
    assert any(c.get("name", "").lower().find("auth") >= 0 for c in state.get("cookies", [])) or state.get("origins"), "Expected some auth storage to persist."
